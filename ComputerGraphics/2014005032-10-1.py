...
def exp(rv):
   theta = l2norm(rv)
   axis = normalized(rv)
   R = np.array([[axis[0]*axis[0]*(1-np.cos(theta)) + np.cos(theta), axis[0]*axis[1]*(1-np.cos(theta)) - axis[2]*np.sin(theta), axis[0]*axis[2]*(1-np.cos(theta)) + axis[1]*np.sin(theta)],
                 [axis[0]*axis[1]*(1-np.cos(theta)) + axis[2]*np.sin(theta), axis[1]*axis[1]*(1 - np.cos(theta)) + np.cos(theta), axis[1]*axis[2]*(1-np.cos(theta)) - axis[0]*np.sin(theta)],
                 [axis[0]*axis[2]*(1-np.cos(theta)) - axis[1]*np.sin(theta), axis[1]*axis[2]*(1-np.cos(theta)) + axis[0]*np.sin(theta), axis[2]*axis[2]*(1-np.cos(theta)) + np.cos(theta)]])
   return R

def log(R):
   theta = np.arccos((R[0,0] + R[1,1] + R[2,2] - 1)/2)
   v = np.zeros(3)
   v[0] = (R[2,1]-R[1,2])/(2*np.sin(theta)) 
   v[1] = (R[0,2]-R[2,0])/(2*np.sin(theta)) 
   v[2] = (R[1,0]-R[0,1])/(2*np.sin(theta)) 
   return theta*v
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2014005032-10-1', None,None)
    if not window:
        glfw.terminate()
        return
