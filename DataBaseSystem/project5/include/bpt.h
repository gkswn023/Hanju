...
typedef struct lock_hash_table{
	list<struct lock_t*> hash_locks[11];
	struct lock_t* last_x[11];
}lock_hash_table;
...
//Concurrency.
int update(int table_id, int64_t key, int64_t* values, int tid, int* reslut);
int begin_tx();
int end_tx(int tid);
