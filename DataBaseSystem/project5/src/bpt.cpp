...
int min_table;
int table_sum = 0;
pthread_mutex_t bm_latch;// global mutex lock for buffer
pthread_mutex_t begin_lock;
pthread_mutex_t hash_latch;
lock_hash_table* lock_table;
trx_t trx_list[100];
int trx_id = 0;
...
int init_db(int num_buf){
    ...
    lock_table = (lock_hash_table*)calloc(1, sizeof(lock_hash_table)*num_buf);

    pthread_mutex_init(&bm_latch, NULL);
    pthread_mutex_init(&hash_latch, NULL);

    for(i=0;i<num_buf;i++){
        pthread_rwlock_init(&manager.buffer_pool[i].buf_latch, NULL);
    }
...
int64_t* find (int table_id, int64_t key, int tid, int* result) {
    ...
    buf_location = find_location(table_id, pagenum);
    pthread_rwlock_rdlock(&manager.buffer_pool[buf_location].buf_latch);
    pthread_mutex_unlock(&bm.latch);
...
int erase(int table_id, int64_t key) {
    ...
    values = find(table_id, key, 0, NULL);
...
int64_t* find (int table_id, int64_t key, int tid, int* result) {
    ...
    buf_location = find_location(table_id, pagenum);
    pthread_rwlock_rdlock(&manager.buffer_pool[buf_location].buf_latch);
    pthread_mutex_unlock(&bm_latch);

    lock_stat = acquire_lock(&trx_list[tid], table_id, pagenum, key, SHARED);

    if(lock_stat == SUCCESS){
        *result = 1;
    }
    else if(lock_stat == DEADLOCK){
        *result = 0;
        end_tx(tid);
        return NULL;
    }
...
int update(int table_id, int64_t key, int64_t* values, int tid, int* result){
    ...
    buf_location = find_location(table_id, pagenum);
    pthread_rwlock_wrlock(&manager.buffer_pool[buf_location].buf_latch);
    pthread_mutex_unlock(&bm_latch);

    lock_stat = acquire_lock(&trx_list[tid], table_id, pagenum, key, EXCLUSIVE);
    ...
    if (i == c->num_keys){
        ...
        memcpy(c->records[i].value, values, sizeof(int64_t)*(ncolumn-1));
        c->is_pinned--;
...
int acquire_lock(trx_t* trx, int table_id, pagenum_t page_id, int64_t key,enum lock_mode mode){
    lock_t* new_lock;
    list<lock_t*> temp; 
    int location;

    pthread_mutex_lock(&hash_latch);
    new_lock = (lock_t*)malloc(sizeof(lock_t));
    new_lock->table_id = table_id;
    new_lock->page_id = page_id;
    new_lock->key = key;
    new_lock->mode = mode;
    new_lock->trx = trx;
    ...
    location = find_location(table_id, page_id);

    if(lock_table[location].hash_locks[table_id].empty()){
        lock_table[location].hash_locks[table_id].push_back(new_lock);
        trx->state = RUNNING;
        if(mode == EXCLUSIVE){
            lock_table[location].last_x[table_id] = new_lock;
        }
        pthread_mutex_unlock(&hash_latch);
        return SUCCESS;
    }
    if(lock_table[location].last_x[table_id] == NULL){
        if(mode == EXCLUSIVE){
            lock_table[location].last_x[table_id] = new_lock;
            pthread_mutex_unlock(&hash_latch);
            return WAIT;
    ...
    else if(deadlock_checker(lock_table[location].last_x[table_id])){
        trx->state = IDLE;
        pthread_mutex_unlock(&hash_latch);
        return DEADLOCK;
    }
        ...
        lock_table[location].hash_locks[table_id].push_back(new_lock); 
        trx->wait_lock = lock_table[location].last_x[table_id];
        trx->state = WAITING;
        pthread_mutex_unlock(&hash_latch);
        return WAIT;
    }
...
int deadlock_checker(lock_t* wait_lock){

    trx_t* trx1 = wait_lock->trx;

    while(trx1->wait_lock != NULL){
        if(trx1->wait_lock == wait_lock){
            return 1;
        }
        trx1 = trx1->wait_lock->trx;
    }
    return 0;
}
...
int begin_tx(){
    ...
    trx_list[trx_id].tid = trx_id;
    trx_list[trx_id].state = IDLE; 
    tmp = trx_id;
    trx_id++;
...
int end_tx(int tid){
    ...
    while(!trx_list[tid].trx_locks.empty()){
        location = find_location(trx_list[tid].trx_locks.front()->table_id,
         trx_list[tid].trx_locks.front()->page_id);
        pthread_rwlock_unlock(&manager.buffer_pool[location].buf_latch);
